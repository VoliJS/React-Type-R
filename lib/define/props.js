...
function onDefine(_a, BaseClass) {
    ...
    if (props) {
        ...
        prototype._props = type_r_1.tools.defaults(props, BaseClass.prototype._props || {});
        var _b = typeSpecs_1.compileSpecs(props), propTypes = _b.propTypes, defaults = _b.defaults, watchers = _b.watchers, changeHandlers = _b.changeHandlers;
        ...
        if (prototype.pureRender) {
            prototype.PropsChangeTokens = pureRender_1.createChangeTokensConstructor(props);
        }
    }
    if (pureRender) {
        this.mixins.merge([pureRender_1.PureRenderMixin]);
    }
}
