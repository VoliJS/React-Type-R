...
var Component = /** @class */ (function (_super) {
    ...
    Component.prototype.transaction = function (fun) {
        ...
        var shouldComponentUpdate = this.shouldComponentUpdate, isRoot = shouldComponentUpdate !== shouldNotUpdate;
        if (isRoot)
            this.shouldComponentUpdate = shouldNotUpdate;
        fun(this.state);
        // Commit transaction...
        if (isRoot) {
...
var AfterRenderMixin = {
    componentDidMount: callAfterRenderHooks,
    componentDidUpdate: callAfterRenderHooks,
    afterRender: function (hook) {
        var _callAfterRender = this._callAfterRender;
        this._callafterRender = _callAfterRender ? function () {
            _callAfterRender();
            hook();
        } : hook;
    }
